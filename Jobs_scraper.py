#!/usr/bin/env python3
# Full job scraper + email sender with debug logging
# Searches Indeed + Internshala, builds HTML email, sends via Gmail App Password
...
def scrape_indeed(url):
    ...
    try:
        ...
        print(f"Found {len(cards)} raw Indeed cards")
        ...
        for c in cards[:10]:
            try:
                ...
                print("Error parsing Indeed card:", e)
                continue
    except Exception as e:
        print("Indeed request failed:", e)

    print(f"Indeed jobs parsed: {len(jobs)}")
    return jobs
...
def scrape_internshala(url):
    ...
    try:
        ...
        print(f"Found {len(cards)} raw Internshala cards")
        ...
        for c in cards[:10]:
            try:
                ...
                print("Error parsing Internshala card:", e)
                continue
    except Exception as e:
        print("Internshala request failed:", e)

    print(f"Internshala jobs parsed: {len(jobs)}")
    return jobs
...
def send_email(subject, html_body):
    try:
        ...
        msg.attach(MIMEText("Your client does not support HTML.", "plain"))
        msg.attach(MIMEText(html_body, "html"))

        print(f"Attempting email to {RECIPIENT_EMAIL}")
...
if __name__ == "__main__":
    main()                snippet = c.select_one(".job-snippet").get_text(" ", strip=True)
